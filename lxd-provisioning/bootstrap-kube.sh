...
yum install -y -q kubelet-1.15.3 kubeadm-1.15.3 kubectl-1.15.3 >/dev/null 2>&1
...
then
  ...
  kubeadm init --kubernetes-version v1.15.3 --image-repository registry.aliyuncs.com/google_containers --pod-network-cidr=10.244.0.0/16 --ignore-preflight-errors=all >> /root/kubeinit.log 2>&1
